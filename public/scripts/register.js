...
registerBtn.addEventListener("click", function (e) {
    ...
    if (registerForm.reportValidity() == false) {
        return false;
    }
    ...
    console.log(flag);
    if (flag == true) {
        axios.post("/registration", formData).then((response) => {
...
email.addEventListener("focusout", (e) => {
    ...
    axios.get("/getUserEmail").then((response) => {
...
registerForm.addEventListener("focusout", (e) => {
    ...
    if (phoneNum.value.length == 0) {
        phoneNum.classList.remove("border-success");
    } else {
        phoneNum.classList.add("border-success");
    }
    if (bdate.value.length == 0) {
        bdate.classList.remove("border-success");
    } else {
        bdate.classList.add("border-success");
    }
...
passwordForm.addEventListener("focusout", (e) => {
    ...
    if (password.value.length > 0 && confirmPassword.value.length > 0 && passValidation() == true && confirmPasswordValidation() == true) {
...
function confirmPasswordValidation() {
    const passw = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,20}$/;
    if (confirmPassword.value.length > 0 && confirmPassword.value.match(passw) == null) {
        passAlert1.style.display = "none";
        passAlert2.style.display = "block";
        confirmPassword.classList.remove("border-success");
        confirmPassword.classList.add("border-danger");
